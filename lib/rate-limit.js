...
module.exports = function createRateLimitChecker(CORSANYWHERE_RATELIMIT) {
  ...
  const maxRequestsPerPeriod = parseInt(rateLimitConfig[1], 10);
  const periodInMinutes = parseInt(rateLimitConfig[2], 10);
  let unlimitedPattern = rateLimitConfig[3]; // Will become a RegExp or void.
  if (unlimitedPattern) {
    const unlimitedPatternParts = [];
    unlimitedPattern.trim().split(/\s+/).forEach((unlimitedHost, i) => {
